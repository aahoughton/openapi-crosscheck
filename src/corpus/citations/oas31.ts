...
/**
 * Specification text, quoted verbatim from the OpenAPI 3.1.1 source document.
 *
...
const SPEC = "https://spec.openapis.org/oas/v3.1.1.html";
...
function cite(anchor: string, quoted: string): Citation {
  return { oasVersion: "3.1", anchor, url: `${SPEC}#${anchor}`, quoted };
}
...
/**
 * The sentence that takes nullability off the serialization surface.
 *
 * If null is undefined for expansion, and an undefined variable is ignored by
 * the expansion process, then a null-valued parameter has no wire form: it
 * serializes to nothing, which is what an absent parameter also serializes to.
 * Nullability therefore changes nothing about how a non-null value is written,
 * and a serialization map has no cell for it to occupy.
 */
